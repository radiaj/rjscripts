...
FILENAME=$1
count=0
myREFDir="/RQexec/johnsonr/Homo_sapiens/UCSC/hg19/Sequence/Chromosomes/female_only"

# Create the reference fasta file index (only needs to be run once)
 cd /RQexec/johnsonr/Homo_sapiens/UCSC/hg19/Sequence/Chromosomes/female_only
 samtools faidx /RQexec/johnsonr/Homo_sapiens/UCSC/hg19/Sequence/Chromosomes/female_only/hg19.fa
...
while read mySIZE
        do
                ...
                cd /RQexec/johnsonr/PARK_LAB/STAR/"${mySIZE}"_STAR2
                ...
                java -Xmx8g -jar $HOME/snpEff/snpEff.jar count hg19 dedupped.bam  -c $HOME/snpEff/snpEff.config > "${mySIZE}".countReads.txt

                # Could not get these steps to run on the cluster; Run from ..._STAR2/
