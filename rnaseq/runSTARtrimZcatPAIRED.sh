...
while read mySIZE
        do
                ...
                # Run STAR
                STAR --genomeDir /RQexec/johnsonr/Homo_sapiens/UCSC/hg19/Sequence/Chromosomes  --quantMode GeneCounts --runThreadN 4 --outSAMtype BAM SortedByCoordinate --readFilesIn "${myDIR}"/"${mySIZE}"*1.f*q.gz "${myDIR}"/"${mySIZE}\
"*2.f*q.gz --readFilesCommand zcat --outFileNamePrefix "${mySIZE}"

                # Create index file for IGV viewer
